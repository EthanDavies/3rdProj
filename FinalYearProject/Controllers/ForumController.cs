...
namespace FinalYearProject.Controllers
{
    ...
    public class ForumController : Controller
    {
        ...
        {
            _forumRepository = new ForumRepository(new BlogDbContext());
        }
        ...
        public ForumController(IForumRepository blogRepository)
        {
            _forumRepository = blogRepository;
        }
            ...
            try
            {
                foreach (var post in posts)
                {
                    ...
                    postList.Add(new ForumViewModel() {
                        ...
                        Title = post.Title, ShortDescription = post.ShortDescription,
                        PostOn = post.PostedOn, Id = post.Id,
        ...
        public ActionResult Comments(PostViewModel model, Post post, string sortOrder)
        {
            ...
            var postComments = _forumRepository.GetPostComments(post).OrderByDescending(d => d.DateTime).ToList();
            try
            {
                foreach (var comment in postComments)
                {
                    if (comment.Replies != null) comment.Replies.Clear();
                    List<CommentViewModel> replies = _forumRepository.GetParentReplies(comment);
                    foreach (var reply in replies)
                    {
                        var rep = _forumRepository.GetReplyById(reply.Id);
        ...
        public ActionResult NewComment(string commentBody, string comUserName, string slug, string postid)
        {
            try
            {
                ...
                else
                {
                    ...
                    int num = 0;
                    var comments = _forumRepository.GetComments().ToList();
                    if (comments.Count() != 0)
                    {
                    ...
                    _forumRepository.AddNewComment(comment);
                }
            } catch (Exception e)
            {
                Console.WriteLine(e);
        ...
        public ActionResult NewParentReply(string replyBody, string comUserName, string postid, string commentid, string slug)
        {
            try
            {
                ...
                if (!comDelChck)
                {
                    List<int> numlist = new List<int>();
                    int num = 0;
                    var replies = _forumRepository.GetReplies().ToList();
                    if (replies.Count != 0)
                    {
                    ...
                    _forumRepository.AddNewReply(reply);
                }
            } catch (Exception e)
            {
                Console.WriteLine(e);
        ...
        public async Task<ActionResult> DeleteReply(CommentViewModel model, string replyid)
        {
            ...
            var reply = _forumRepository.GetReplyById(replyid);
            ...
            {
                return RedirectToAction("Post", new { slug = _forumRepository.GetPosts().Where(x => x.Id == reply.PostId).FirstOrDefault().UrlSeo });
            }
        }
        ...
        {
            var reply = _forumRepository.GetReplyById(replyid);
            var repliesList = _forumRepository.GetChildReplies(reply);
            var postid = reply.PostId;
            try
            {
                if (repliesList.Count() == 0)
                {
                    _forumRepository.DeleteReply(replyid);
                }
                else
                {
            ...
            return RedirectToAction("Post", new { slug = _forumRepository.GetPosts().Where(x => x.Id == postid).FirstOrDefault().UrlSeo });
        }
        ...
        public ActionResult NewChildReply(string preplyid, string comUserName, string replyBody)
        {
            ...
            var preply = _forumRepository.GetReplyById(preplyid);
            try
            {
                if (!repDelCheck)
                {
            ...
            return RedirectToAction("Post", new { slug = _forumRepository.GetPosts().Where(x => x.Id == preply.PostId).FirstOrDefault().UrlSeo });
        }
        ...
        {
            var user = await GetCurrentUserAsync();
            var comment = _forumRepository.GetCommentById(commentid);
            ...
            {
                return RedirectToAction("Post", new { slug = _forumRepository.GetPosts().Where(x => x.Id == comment.PostId).FirstOrDefault().UrlSeo });
            }
        ...
        public async Task<ActionResult> DeleteComment(CommentViewModel model, string commentid)
        {
            ...
            else
            {
                return RedirectToAction("Post", 
                    new { slug = _forumRepository.GetPosts().Where(x => x.Id == comment.PostId).FirstOrDefault().UrlSeo });
            }
        }
        ...
        public ActionResult DeleteComment(string commentid)
        {
            ...
            var repliesList = _forumRepository.GetParentReplies(comment);

            try
            {
                if (repliesList.Count() == 0)
                {
                    _forumRepository.DeleteComment(commentid);
                }
                else
                {
                    comment.DateTime = DateTime.Now;
            ...
            return RedirectToAction("Post",
                new { slug = _forumRepository.GetPosts().Where(x => x.Id == postid).FirstOrDefault().UrlSeo });
        }
        ...
        {
            return _forumRepository.GetChildReplies(parentReply);
        }
        ...
        {
            return _forumRepository.CommentDeleteCheck(commentid);
        }
        ...
        {
            return _forumRepository.ReplyDeleteCheck(replyid);
        }
        ...
        {
            return _forumRepository.GetPosts();
        }
        ...
        {
            return _forumRepository.GetPostCategories(post);
        }
        ...
        {
            foreach (var ct in _forumRepository.GetCategories())
            {
        ...
        public PostViewModel CreatePostViewModel(string slug)
        {
            ...
            var postid = _forumRepository.GetPostIdBySlug(slug);
            var post = _forumRepository.GetPostById(postid);
            try
            {
                model.ID = postid;
                ...
                model.PostCategories = _forumRepository.GetPostCategories(post).ToList();
        ...
        {
            bool flag = false;
            var usernames = _forumRepository.GetSeenUsernames(postid);
            try
            {
            ...
            try
            {
                ...
                else
                {
                    if (user.Role.Equals("Student"))
                    {
                        ...
                        List<int> numlist = new List<int>();
                        int num = 0;
                        var posts = _forumRepository.GetPostsSeen();
                        if (posts.Count != 0)
                        {
                            foreach (var post in posts)
                            {
                        ...
                        model.Title = title;
                        _forumRepository.AddNewSeen(model);
        ...
        public ActionResult PostSeen(string postid)
        {
            ...
            List<ApplicationUser> users = _forumRepository.GetPostUsers(postid);
            List<string> haveSeenPostUsername = new List<string>();
            List<string> allStudentUsernames = _forumRepository.GetAllStudentUsers();
        ...
        public ActionResult EditPost(PostViewModel model)
        {
            ...
            var post = _forumRepository.GetPostById(model.ID);
            try
            {
        ...
        public ActionResult AddCategoryToPost(string postid)
        {
            PostViewModel model = new PostViewModel();
            model.ID = postid;
            model.Categories = _forumRepository.GetCategories();
            return View(model);
        }
        ...
        {
            var post = _forumRepository.GetPostById(model.ID);
            var postCats = _forumRepository.GetPostCategories(post);
            ...
            try
            {
                ...
                if (!pCatIds.SequenceEqual(nCatIds))
                {
                    foreach (var pCat in postCats)
                    {
                        _forumRepository.RemovePostCategories(model.ID, pCat.Id);
                    }
                    foreach (var cat in model.Categories)
                    {
                        ...
                        if (cat.Checked == true)
                        {
                            ...
                            _forumRepository.AddPostCategories(postCategory);
                        }
                    }
                    _forumRepository.Save();
                }
            ...
            return RedirectToAction("Post", "Forum", new { slug = post.Title + post.Id });
        }
        ...
        public ActionResult RemoveCategoryFromPost(string slug, string postid, string catname)
        {
            ...
            _forumRepository.RemoveCategoryFromPost(postid, catname);
        ...
        public ActionResult AddNewCategory(string postid, string catName, string catUrlSeo, string catDesc)
        {
            ...
            {
                catUrlSeo = catName;
                _forumRepository.AddNewCategory(catName, catUrlSeo, catDesc);
                return RedirectToAction("AddCategoryToPost", new { postid = postid });
            }
            else
            {
                _forumRepository.AddNewCategory(catName, catUrlSeo, catDesc);
                return RedirectToAction("Categories", "Forum");
            }
        }
        ...
        public ActionResult Categories(string[] categoryNames)
        {
            if (categoryNames != null)
            {
                ...
                {
                    var category = _forumRepository.GetCategories().Where(x => x.Name == catName).FirstOrDefault();
                    _forumRepository.RemoveCategory(category);
                }
            }          
            return RedirectToAction("Categories", "Forum");
        }
        ...
        {
            _forumRepository.DeletePostAndComponents(postId);
        ...
        public ActionResult AddNewPost(PostViewModel model)
        {
            ...
            return RedirectToAction("AddCategoryToPost", "Forum", new { postid = model.ID });
        }
